...
from PyQt6.QtWidgets import (
    ...
    QLabel, QLineEdit, QPushButton, QMessageBox, QComboBox, QCheckBox, QSpacerItem, QListWidget, QAbstractItemView, QSystemTrayIcon, 
    QMenu, QTextEdit
)
import time
...
class App(QWidget):
    debug = True # Enable debug only if working in vscode ; Disable debug if putting into production
    if debug: relative_path = "bin/aninotify_icon.ico"
    else: relative_path = "aninotify_icon.ico"
    try:
    ...
    def setup_tray(self):
        ...
        self.show_debug_action.triggered.connect(lambda: self.output_window.on_click())
